...
final class PerkModel {
    static var perkTitle: String = ""
    static var time: String = ""
    
    // 0 0 : 0 0 : 0 0
    // 0 1 2 3 4 5 6 7
    static var totalSecFromSession: Int64 = 0
    ...
    static func calcTotalSecFromSession() {
        totalSecFromSession = Int64("\(time[0])\(time[1])")! * 3600 + Int64("\(time[3])\(time[4])")! * 60 + Int64("\(time[6])\(time[7])")!
    }
}

final class PerkTimer {
    static var timerIsActive = false
    ...
    static var timeBeforeBackground = Date()
    static var timeAfterBackground = Date()
